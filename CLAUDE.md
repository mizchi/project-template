...
The devShell provides: `pkf` (task runner), `pkspec` (Pkl-based spec/test
runner), `apm` (skill/prompt distribution), `pkl` (Taskfile engine), `gitleaks`
(secret scan), `ast-grep` (structural search). There is intentionally **no**
language runtime.
...
pkf run spec       # pkspec: regenerate schemas/ + exec Test.pkl
...
- APM: `apm.yml` + `apm.lock.yaml` are tracked; deployed skills under
  ...
  reproducible.
- Tests: author them in `Test.pkl` (tracked). The `schemas/` dir it amends is
  regenerated by `pkspec init` (run automatically by `pkf run spec`) and is
  gitignored. A `Test` picks one body — `CmdTest { cmd }`,
  `SequentialTest { steps }`, or `ParallelTest { parallelSteps }`. The shipped
  `smoke` test keeps the `ci` gate green until you add real ones.
...
2. Run `skill-selector` to pick the skills this repo needs, then append them to
   `apm.yml`.
3. Add build / test / lint tasks to `Taskfile.pkl` as `deps` of `ci`, and write
   real tests in `Test.pkl` (the `spec` task runs them).
