...
# It is easy to forget to run the bootstrap by hand, so the very first tool
# call hits "nix: command not found" and the whole toolchain (pkf / apm / pkl /
...
if [ "${CLAUDE_CODE_REMOTE:-}" = "true" ]; then
  # ccweb-init.sh is idempotent and self-healing: installs single-user nix when
  ...
  .claude/scripts/ccweb-init.sh >&2
fi
